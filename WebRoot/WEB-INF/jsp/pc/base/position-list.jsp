...
<div>
  <div class="layui-btn-group demoTable">
    ...
    <at:popedom cid="23">
    <button class="layui-btn " data-type="authCheckData">配置权限</button>
    </at:popedom>
    <at:popedom cid="22">
...
<div class="layui-form-item demoTable" style="margin-top: 15px;">
  <div class="layui-input-inline">
    <input type="text" id="name" lay-verify="title" autocomplete="on" placeholder="职务名称" class="layui-input">
  </div>
...
<script>
  layui.use('table', function() {
      ...
      table.render({
          elem: '#LAY_table_user'
          , url: 'position!list.action?flag=json'
          , cols: [[
      ...
      var $ = layui.$, active = {
          ...
          getCheckData: function(){ //获取选中数据
              ...
              closeBtn: 1,
              shadeClose: false,
              content: 'position!edit.action?flag=edit&object.id='+data[0].id
              });
          },
          delCheckData:function () {
              ...
              layer.confirm('真的删除行么', function(index){
                  layer.close(index);
                  //向服务端发送删除指令
                  startRequest("position!del.action", function(text){
                      layer.alert(text);
                      window.location.reload();
                      }, {"object.id":data[0].id});
              });
          },authCheckData: function(){ //获取选中数据
              var checkStatus = table.checkStatus('testReload')
                  ,data = checkStatus.data;
              console.info(data.length);
              if(data.length>1) {
              ...
              layer.open({
                  ...
                  closeBtn: 1,
                  shadeClose: false,
                  content: 'position!auth.action?id='+data[0].id
              });
          }
      };
  ...
  function add() {
      layer.open({
          ...
          closeBtn: 1,
          shadeClose: false,
          content: 'position!edit.action?flag=add'
      });
  }
</script>
