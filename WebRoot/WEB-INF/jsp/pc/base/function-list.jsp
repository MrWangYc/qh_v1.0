...
<div class="layui-form-item demoTable" style="margin-top: 15px;">
  <div class="layui-input-inline">
    <input type="text" id="name" lay-verify="title" autocomplete="on" placeholder="功能/模块名称" class="layui-input">
  </div>
  <div class="layui-input-inline">
...
<script>
  layui.use('table', function() {
      ...
      table.render({
          elem: '#LAY_table_user'
          , url: 'function!list.action?flag=json&id=${id}'
          , cols: [[
              {checkbox: true, fixed: true}
              , {field: 'id', title: '编码', width: 80, sort: true}
              , {field: 'type', title: '类型', width: 100, sort: true}
              , {field: 'functionName', title: '名称', width: 180, sort: true}
              , {field: 'url', title: '地址', width: 250}
              , {field: 'orderNo', title: '顺序', sort: true, width: 80}
              , {field: 'status', title: '状态', width: 80}
              , {field: 'validate', title: '验证', sort: true, width: 80}
              , {field: 'cache', title: '是否记录', sort: true, width: 120}
              , {field: 'isShow', title: '是否显示', sort: true, width: 120}
          ]]
          , id: 'testReload'
          , page: true
          , height: 721
      ...
      var $ = layui.$, active = {
          ...
          getCheckData: function(){ //获取选中数据
              ...
              closeBtn: 1,
              shadeClose: false,
              content: 'function!edit.action?flag=edit&object.id='+data[0].id
              });
          },
          delCheckData:function () {
              ...
              layer.confirm('真的删除行么', function(index){
                  layer.close(index);
                  //向服务端发送删除指令
                  startRequest("function!del.action", function(text){
                      layer.alert(text);
  ...
  function add() {
      layer.open({
          ...
          closeBtn: 1,
          shadeClose: false,
          content: 'function!edit.action?flag=add&object.parent.id=${id}'
      });
  }
</script>
