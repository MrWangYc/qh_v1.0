...
<div style="display: flex;">
  ...
  <div style="flex: 1;height: 830px;margin-left: 2px;" id="demo">
  </div>
</div>
...
<script>
            ...
            ,nodes: [ //节点
                {name:'系统基础编码'
                    ...
                    <s:iterator status="v" value="#request.fList">
                      {name: '${name}'
                          ,id: ${id}
                          <s:if test="#request.childCodes.size()>0">
                            ,children: [
                              <s:iterator value="#request.childCodes" status="s">
                                {
                                    name: '${name}'
                                    ,id: ${id}
                                },
    ...
    function loadPage(id) {
        if(id){
            var f= ' <iframe width="100%" height="100%" name="fList" id="fList" scrolling="auto" src="sys-code!list.action?flag=list&id='+id+'" frameborder="0"></iframe>';
            var fIframe = $("#fList");
            if(fIframe){
                fIframe.remove();
            }
